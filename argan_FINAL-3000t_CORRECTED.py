...
# ── CAPACITY SCENARIOS (Section 2.3 of fixing instructions) ──────────────────
CAPACITY_SCENARIOS = {
    'Small cooperative-industrial': 1000,    # t/yr
    'Base regional plant':          3000,    # t/yr  ← BASE CASE
    'Upper regional plant':         5000,    # t/yr
    'Expansion stress-test':       10000,    # t/yr  (requires strict governance)
}

def capacity_to_kgh(cap_ty, hours=8000):
    """Convert annual capacity (t/yr) to hourly feed rate (kg/h)."""
    return cap_ty * 1000 / hours

# Base-case capacity: 3,000 t/yr sun-dried whole argan fruit equivalent
CAP_TY = 3000                     # t/yr
CAP    = capacity_to_kgh(CAP_TY)  # = 375.0 kg/h at 8,000 h/yr   # kg/h total fruit
...
class CHP(bst.Unit):
    ...
    def _design(self):
        self.design_results['Elec_gross_kW']  = self._pwr   # GROSS electricity before internal load
        self.design_results['Heat_kW']        = self._heat
        self.design_results['Elec_kW']        = self._pwr   # alias kept for compatibility

    # CHP energy balance: store all terms for Table 4.1
    @property
    def chp_energy_table(self):
        bg, sg = self.ins
        tot_ch4 = bg.imass['Methane'] + sg.imass['Methane']
        fuel_kW = tot_ch4 * 50050 / 3600
        return {
            'AD_CH4_kg_h':     float(bg.imass['Methane']),
            'Pyro_CH4_kg_h':   float(sg.imass['Methane']),
            'Total_CH4_kg_h':  float(tot_ch4),
            'Fuel_energy_kW':  float(fuel_kW),
            'Gross_elec_kW':   float(self._pwr),
            'Heat_recov_kW':   float(self._heat),
            'Fuel_util_pct':   float((self._pwr + self._heat) / fuel_kW * 100),
        }
...
print(f"  Capacity basis   : {CAP_TY} t/yr ({CAP:.0f} kg/h at {H} h/yr)")
...
# Feedstock: sun-dried whole argan fruit equivalent.
# Base: $0.60/kg (MAD 6.1/kg). Sensitivity: $0.30–$0.90/kg.
# Rationale: FAO (2022) reports cooperative procurement of dried fruit at
# MAD 4–10/kg depending on region and year. $0.12/kg ($0.012/kg) was
# the fresh/wet fruit residue price — incorrect for dried processed input.
# Do NOT label $0.12/kg as the main feedstock price.
FEEDSTOCK_PRICE_BASE  = 0.60   # USD/kg  (base case, dried fruit equivalent)
FEEDSTOCK_PRICE_RANGE = [0.30, 0.60, 0.90]   # USD/kg sensitivity range
opex_feedstock = CAP * H * FEEDSTOCK_PRICE_BASE
...
#   system from falsely "low-carbon" to correctly "low-carbon".
...
# Solvent: EtOH with recovery sensitivity (Section 5.3 of fixing instructions)
# Case A (conservative/base): 100% input charged — no recovery unit modelled
# Case B (industrial recovery): 95% recovery; only 5% makeup charged to LCA
def calculate_lca_gwp(ethanol_recovery=0.0, net_export_kWh_val=None, FU_val=None,
                       gwp_e_val=0, gwp_t_val=0, gwp_w_val=0):
    """Return (GWP_kg, breakdown_dict) for given EtOH recovery and conditions."""
    if net_export_kWh_val is None: net_export_kWh_val = net_export_kWh
    if FU_val is None: FU_val = FU
    etoh_in  = 180 * H if (not hasattr(calculate_lca_gwp,'_etoh')) else calculate_lca_gwp._etoh
    etoh_chrg= etoh_in * (1 - ethanol_recovery)
    gwp_s    = etoh_chrg * EF['ethanol_kg']
    gwp_ec   = -net_export_kWh_val * EF['grid_kWh']
    total    = gwp_e_val + gwp_t_val + gwp_s + gwp_w_val + gwp_ec
    return total / FU_val, {'gwp_elec': gwp_e_val/FU_val, 'gwp_transp': gwp_t_val/FU_val,
                              'gwp_etoh': gwp_s/FU_val, 'gwp_water': gwp_w_val/FU_val,
                              'gwp_elec_cr': gwp_ec/FU_val}

EtOH_total = 180 * H   # kg/yr — full solvent flow; recovery sensitivity computed below
...
GWP_kg = GWP_total   / FU   # corrected GWP100 per kg oil (Case A: no EtOH recovery)

# ── EtOH recovery sensitivity (Cases A & B per fixing instructions §5.3) ─────
# Case A = GWP_kg (already calculated above — 100% EtOH charged)
# Case B — 95% EtOH recovery (industrial distillation-recycle scenario)
EtOH_makeup_B   = EtOH_total * 0.05   # kg/yr only makeup
gwp_etoh_B      = EtOH_makeup_B * EF['ethanol_kg']
GWP_B_total     = gwp_elec + gwp_transp + gwp_etoh_B + gwp_water + gwp_elec_cr
GWP_kg_B        = GWP_B_total / FU   # kg CO2-eq/kg oil (with 95% EtOH recovery)
print(f"  EtOH Case A (no recovery): GWP = {GWP_kg:.3f} kg CO2-eq/kg oil")
print(f"  EtOH Case B (95% recovery): GWP = {GWP_kg_B:.3f} kg CO2-eq/kg oil")
...
R = {
    ...
    'GWP_kg_CaseA' : GWP_kg,
    'GWP_kg_CaseB' : GWP_kg_B,
    'CAP_TY'       : CAP_TY,
    'FEEDSTOCK_PRICE_BASE': FEEDSTOCK_PRICE_BASE,
    # PCE breakdown
    'PCE_breakdown': {k: float(v) for k, v in PCE_breakdown.items()},
    'chp_table': U07.chp_energy_table,
    'GWP_kg_CaseA': GWP_kg,
    'GWP_kg_CaseB': GWP_kg_B,
    'EtOH_recovery_sensitivity': {
        'Case_A_no_recovery':   {'ethanol_recovery': 0.00, 'GWP_kg': GWP_kg},
        'Case_B_95pct_recovery':{'ethanol_recovery': 0.95, 'GWP_kg': GWP_kg_B},
    },
    'CAP_TY': CAP_TY,
    'FEEDSTOCK_PRICE_BASE': FEEDSTOCK_PRICE_BASE,
}
...
streams_table = [
    ...
    (1,  'Dried argan fruit feed', 25, 101, '375 ',  'Multi',         'S'),
...
ax.text(tb_x+tb_w/2, tb_y+tb_h-0.62,
        'Process Flow Diagram  —  Capacity: 3 000 t/yr argan fruit (375 kg/h, base case)',
...
fig3.suptitle('Life Cycle Assessment — GWP100 Results\n'
               '(Integrated Argan Biorefinery, 3,000 t/y dried fruit equivalent, Ecoinvent 3.9 cutoff)',
...
sens = {
    # FIX: oil price ±20% affects oil revenue only, NOT total revenue
    # Use rev_oil (oil-only component), not REV (total)
    'Oil price ±20%\n($48–$72/kg)':        (calc_npv(CAP_T,REV_T+rev_oil*0.20,OPEX_T,DR,LIFE),
                                              calc_npv(CAP_T,REV_T-rev_oil*0.20,OPEX_T,DR,LIFE)),
    'CAPEX ±30%':                            (calc_npv(CAP_T*0.70,REV_T,OPEX_T,DR,LIFE),
                                              calc_npv(CAP_T*1.30,REV_T,OPEX_T,DR,LIFE)),
    'Feedstock cost ±50%\n($0.30–$0.90/kg)':(calc_npv(CAP_T,REV_T,OPEX_T-R['opex_feedstock']*0.5,DR,LIFE),
                                             calc_npv(CAP_T,REV_T,OPEX_T+R['opex_feedstock']*0.5,DR,LIFE)),
...
gwp_abs = R['GWP_kg']  # use real sign — positive GWP must plot positive
...
add_para(doc,
    f'At 3,000 t/yr sun-dried whole argan fruit (8,000 h/yr), the biorefinery produces '
    ...
    f'kg CO2-eq/kg oil (positive but comparatively low; dominant driver is ethanol solvent input), FED = '
...
add_para(doc,
    f'The integrated argan biorefinery processes 3,000 t/yr whole argan fruit (375 kg/h '
...
add_para(doc,
    ...
    'Because CH4 generated in anaerobic digestion and pyrolysis-derived gas is combusted internally in the CHP unit, no natural-gas substitution credit was assigned (avoids double-counting). '
...
make_table(doc,
    ...
    rows=[
        ['Argan oil (cold-press)',    f2(R['oil_flow'],1),     f'{f2(R["oil_flow"]*H/1000,1)} t/yr', '$60/kg (base)'],
    ...
    caption='Table 2. Mass balance and product summary at 3,000 t/y sun-dried whole argan fruit equivalent (375 kg/h, 8,000 h/y).')
...
add_para(doc,
    ...
    f'$60/kg base selling price (bulk commercial assumption). NPV = ${f2(R["NPV"]/1e6,0)} M at 10% WACC; '
    f'IRR = {f2(R["IRR"],1)}%; payback = {f2(R["PB"],2)} yr.', indent=True)
add_para(doc, 
    'These economic results assume full market absorption of approximately '
    f'{f2(R["oil_flow"]*H/1000,0)} t/yr of argan oil at the $60/kg bulk price. '
    'This assumption requires validation through offtake agreements, certification status, '
    'export-channel analysis, and cooperative procurement contracts. The LCOP is defined as '
    'annual operating cost divided by annual argan oil output and does not include capital '
    'recovery; it should not be interpreted as a minimum selling price (MSP).', indent=True)
...
add_para(doc,
    ...
    f'NPV driver. A ±20% change around the $60/kg base oil price (i.e. $48–$72/kg) is the dominant NPV swing. '
    f'CAPEX uncertainty (±30%) is secondary. The wide gap between LCOP '
    f'(${f2(R["LCOP"],2)}/kg) against the $60/kg base selling price represents a scenario-based '
    f'risk buffer. Saponin price escalation to pharmaceutical grade ($80–150/kg) could '
    f'add substantial upside to NPV.', indent=True)
...
add_para(doc,
    ...
    f'{f2(R["GWP_kg"],4)} kg CO2-eq/kg argan oil is positive but comparatively low — '
    f'the process should be described as comparatively low-carbon under the stated assumptions. The electricity export credit ({f2(R["gwp_cu"],4)} kg CO2-eq/kg oil) '
...
add_figure(doc, os.path.join(OUT,'fig3_LCA.png'),
           ...
           'System expansion (ISO 14044 preferred) is the recommended approach per ISO 14044 §4.3.4.', 6.2)
...
add_figure(doc, os.path.join(OUT,'fig5_LCA_benchmarking.png'),
           ...
           'Argan biorefinery achieves a comparatively lower carbon footprint on all three indicators.', 6.2)
...
for txt in [
    ...
    f'and {f2(R["elec_kw"],0)} kW electricity from 3,000 t/yr fruit — valorising 100% vs 15% '
    ...
    f'comparatively low GWP under the stated assumptions — sensitive to EtOH recovery, co-product treatment, and electricity-credit assumptions.',
...
# ═══════════════════════════════════════════════════════════════════════════════
# 11B.  SUPPLEMENTARY INFORMATION TABLES  (exported as CSV)
#       Per Section 10 of fixing instructions (Tables S1–S10)
# ═══════════════════════════════════════════════════════════════════════════════
import pandas as pd

print("Generating supplementary information tables (S1–S10)...")

# ── Table S1: Stream table ────────────────────────────────────────────────────
with open(os.path.join(OUT, 'res.json')) as _f:
    _R = json.load(_f)

_H = _R['H']; _CAP = 375.0  # 3000 t/yr

_stream_data = [
    ('S1',  'Dried whole argan fruit feed', 375.0,   'Multi-component',        25, 101, 'S',    'Splitter'),
    ('S2',  'Kernel fraction',              56.25,   'Oil, Prot, Saponin',      25, 101, 'S',    'P-101 Cold Press'),
    ('S3',  'Pulp fraction',                281.25,  'Glucose, Cell, H₂O',      25, 101, 'S/L',  'R-101 Digester'),
    ('S4',  'Shell fraction',               37.5,    'Lignin, Cellulose, Ash',   25, 101, 'S',    'K-101 Pyrolysis'),
    ('S5',  'Crude oil (press effluent)',    _R['oil_flow']+5.0, 'Triolein, H₂O', 30, 101, 'L', 'F-101 Filter'),
    ('S6',  'Press cake',                   37.0,   'Protein, Saponin, Cell',   25, 101, 'S',    'E-101 Extractor'),
    ('S7',  'Argan oil product (PRODUCT)',  _R['oil_flow'],  'Triolein (92% rec.)', 30, 101, 'L', 'Storage'),
    ('S9',  'EtOH/H₂O solvent (INPUT)',    300.0,    '60 wt% EtOH/H₂O',         60, 101, 'L',    'E-101 Extractor'),
    ('S10', 'Saponin extract',              95.0,   'Saponin, EtOH, H₂O',      60, 101, 'L',    'E-102 Evaporator'),
    ('S11', 'Saponin concentrate (PRODUCT)',_R['sap_flow'], 'Saponin (~85% rec.)', 70, 101, 'L', 'Storage'),
    ('S12', 'EtOH condensate (recycle*)',   85.0,   'EtOH, H₂O',               70, 101, 'L',    'E-101 or waste'),
    ('S13', 'Biogas (AD → CHP)',            _R['ch4_flow']+10.0, 'CH₄ 65%, CO₂ 35%', 35, 110, 'G', 'G-101 CHP'),
    ('S14', 'Digestate biofertiliser (PROD)', _R['dg_flow'],'N,P,K,H₂O COD',    35, 101, 'L',   'Storage/land appl.'),
    ('S15', 'Pyrolysis syngas (→ CHP)',      13.7,   'CH₄, CO₂, CO, H₂',       500, 110, 'G', 'G-101 CHP'),
    ('S16', 'Biochar (PRODUCT)',            _R['bc_flow'], 'C>70%, Ash',          500, 101, 'S',  'Char silo'),
    ('S17', 'Bio-oil (PRODUCT)',            _R['bio_oil_flow'],'Phenolics,H₂O',  25, 101, 'L',   'Storage'),
    ('S18', 'Net electricity (PRODUCT)',    str(f"{_R['elec_kw']:.0f} kW"), '—', None, None,'Elec', 'Grid'),
    ('S21', 'Flue gas (to atmosphere)',     60.0,   'CO₂, H₂O',                150, 101, 'G',  'Atmosphere'),
]
_cols = ['Stream','Description','Flow (kg/h)','Main components','T (°C)','P (kPa)','Phase','Destination']
pd.DataFrame([dict(zip(_cols, r)) for r in _stream_data]).to_csv(
    os.path.join(OUT, 'SI_Table_S1_stream_table.csv'), index=False)
print("  ✓ SI Table S1 (stream table)")

# ── Table S2: Feedstock composition ──────────────────────────────────────────
_comp = [
    ('Pulp (75 wt%)',   'Glucose',   33.75, 'hexoses',         'Khalil & Haidar (2016)'),
    ('Pulp (75 wt%)',   'Cellulose', 18.75, 'structural carb.','Ait Itto et al. (2024)'),
    ('Pulp (75 wt%)',   'Water',     15.0,  'moisture',        'Khalil & Haidar (2016)'),
    ('Pulp (75 wt%)',   'Ash',       7.5,   'minerals',        'Khalil & Haidar (2016)'),
    ('Shell (10 wt%)',  'Lignin',    5.5,   'phenolic polymer', 'Zine el Abidine (2013)'),
    ('Shell (10 wt%)',  'Cellulose', 4.0,   'structural',      'Zine el Abidine (2013)'),
    ('Shell (10 wt%)',  'Ash',       0.5,   'minerals',        'Zine el Abidine (2013)'),
    ('Kernel (15 wt%)', 'Triolein',  7.2,   'main triglyceride','El Monfalouti (2010)'),
    ('Kernel (15 wt%)', 'Protein',   4.5,   'seed protein',    'Mirpoor (2024)'),
    ('Kernel (15 wt%)', 'Saponin',   0.75,  'triterpene glyco.','Taarji (2018)'),
    ('Kernel (15 wt%)', 'Water',     1.8,   'moisture',        'El Monfalouti (2010)'),
    ('Kernel (15 wt%)', 'Ash',       0.75,  'minerals',        'El Monfalouti (2010)'),
]
pd.DataFrame(_comp, columns=['Fraction','Component','Mass (kg/100 kg fruit)','Type','Reference']).to_csv(
    os.path.join(OUT,'SI_Table_S2_feedstock_composition.csv'), index=False)
print("  ✓ SI Table S2 (feedstock composition)")

# ── Table S3: Equipment sizing ────────────────────────────────────────────────
_equip = [
    ('P-101','Hydraulic cold press','Kernel 56.25 kg/h','92% oil recovery','25 W/kg feed','El Monfalouti 2010'),
    ('F-101','Plate-frame filter','Crude oil ~24 kg/h','95% water removal','—','Towler & Sinnott 2013'),
    ('E-101','Saponin extractor','Press cake; 90 kg solv.','L:S = 10:1; 60°C','0.5 kW agitation','Taarji 2018'),
    ('E-102','3-effect evaporator','~95 kg/h dilute extract','85% water removal; E=2.8','see design','Towler & Sinnott 2013'),
    ('R-101','Anaerobic CSTR','Pulp 281.25 kg/h; HRT 25d','Y=0.280 Nm³/kg VS; 65% VSd','5W/m³ mixing','Carrere 2010'),
    ('K-101','Rotary kiln pyrolysis','Shell 37.5 kg/h','470–490°C; bc 37.99%','1.4 kJ/g duty','Ait Itto 2024'),
    ('G-101','CHP gas engine','Biogas + syngas CH₄','η_e=35%; η_h=45%','LHV=50.05 MJ/kg','Jenbacher J208'),
]
pd.DataFrame(_equip, columns=['Tag','Equipment','Feed basis','Key parameter','Power/duty','Reference']).to_csv(
    os.path.join(OUT,'SI_Table_S3_equipment_sizing.csv'), index=False)
print("  ✓ SI Table S3 (equipment sizing)")

# ── Table S4: PCE breakdown ───────────────────────────────────────────────────
pce_data = [(k, v) for k,v in _R['PCE_breakdown'].items()]  # already 2026 USD — no re-escalation
pd.DataFrame(pce_data, columns=['Item','PCE 2026 USD']).to_csv(
    os.path.join(OUT,'SI_Table_S4_PCE_breakdown.csv'), index=False)
print("  ✓ SI Table S4 (PCE breakdown)")

# ── Table S5: OPEX assumptions ────────────────────────────────────────────────
_opex = [
    ('Argan fruit feedstock','$0.60/kg (base)','$0.30–$0.90/kg','FAO 2022; dried fruit bulk'),
    ('Labour (12 operators)','$620/month','$500–$900/month','paylab.com Morocco 2024'),
    ('Supervision (20%)','20% of labour','15–25%','Turton et al. 2009 Table 8.2'),
    ('Maintenance','4% FCI/yr','3–5%','Turton et al. 2009'),
    ('Insurance','0.5% FCI/yr','0.4–0.6%','Moroccan industrial norm'),
    ('EtOH solvent makeup','5% of 90 kg/h × $0.60/kg','Case A/B sensitivity','Taarji 2018'),
    ('Utilities (net elec.)','$0.08/kWh','$0.05–$0.12/kWh','ONEE 2023 [32]'),
    ('Chemicals/catalysts','$5,000/month','±50%','Minor consumables'),
    ('Process water','$0.001/kg','fixed','MAD 5–10/m³'),
]
pd.DataFrame(_opex, columns=['Item','Base value','Range','Source']).to_csv(
    os.path.join(OUT,'SI_Table_S5_OPEX_assumptions.csv'), index=False)
print("  ✓ SI Table S5 (OPEX assumptions)")

# ── Table S6: Product prices ──────────────────────────────────────────────────
_prices = [
    ('Argan oil',          '$60/kg',   '$40/kg',  '$120/kg', 'bulk vs premium cosmetic/food'),
    ('Saponin concentrate','$35/kg',   '$10/kg',  '$80/kg',  'technical vs high-value'),
    ('Biochar',            '$0.80/kg', '$0.25/kg','$1.50/kg','agricultural grade'),
    ('Bio-oil',            '$0.25/kg', '$0.10/kg','$0.40/kg','low-grade pyrolysis liquid'),
    ('Digestate',          '$0.02/kg', '$0/kg',   '$0.04/kg','biofertiliser'),
    ('Electricity',        '$0.08/kWh','$0.05/kWh','$0.12/kWh','ONEE HV industrial tariff'),
]
pd.DataFrame(_prices, columns=['Product','Base price','Low case','High case','Justification']).to_csv(
    os.path.join(OUT,'SI_Table_S6_product_prices.csv'), index=False)
print("  ✓ SI Table S6 (product prices)")

# ── Table S7: LCA inventory ───────────────────────────────────────────────────
_lca_inv = [
    ('Grid electricity import', _R.get('net_grid_MWh',0), 'MWh/yr',
     0.547, 'kg CO₂-eq/kWh', 'IEA Morocco 2023 [31]'),
    ('Diesel transport (fruit 150km + oil 500km)',
     (_R.get('gwp_t',0)*1000/3.195 if _R.get('gwp_t',0) else 0),
     'kg/yr', 3.195, 'kg CO₂-eq/kg', 'Ecoinvent 3.9 [29]'),
    ('Ethanol solvent (Case A — no recovery)',
     180*_H, 'kg/yr', 1.52, 'kg CO₂-eq/kg', 'Ecoinvent 3.9 [29]; Pereira 2019 [30]'),
    ('Ethanol solvent (Case B — 95% recovery)',
     180*_H*0.05, 'kg/yr', 1.52, 'kg CO₂-eq/kg', 'Ecoinvent 3.9 [29]; Pereira 2019 [30]'),
    ('Process water', 90*_H, 'kg/yr', 0.00034, 'kg CO₂-eq/kg', 'Ecoinvent 3.9 [29]'),
    ('Net electricity export (credit)',
     -_R.get('net_export_MWh', _R.get('elec_OUT_MWh',0)), 'MWh/yr',
     -0.547, 'kg CO₂-eq/kWh', 'System expansion ISO 14044 §4.3.4'),
]
pd.DataFrame(_lca_inv, columns=[
    'Inventory item','Annual amount','Unit','Emission factor','EF unit','Source']).to_csv(
    os.path.join(OUT,'SI_Table_S7_LCA_inventory.csv'), index=False)
print("  ✓ SI Table S7 (LCA inventory)")

# ── Table S8: LCA allocation sensitivity ─────────────────────────────────────
_total_mass_yr = (_R['oil_flow']+_R['sap_flow']+_R['bc_flow']+
                  _R['bio_oil_flow']+_R['ch4_flow']+_R['dg_flow'])*_H
_oil_mass_frac = _R['oil_flow']*_H / _total_mass_yr
_oil_rev_frac  = _R['rev_oil'] / _R['REV']
_gross_gwp_A   = _R['gwp_eu']+_R['gwp_tu']+_R['gwp_su']+_R['gwp_wu']  # per kg oil, pos. only
_alloc = [
    ('System expansion (base, ISO preferred)', _R['GWP_kg_CaseA'], 'No allocation — system expanded'),
    ('Mass allocation',  _gross_gwp_A * _oil_mass_frac, f'{_oil_mass_frac*100:.1f}% mass fraction'),
    ('Economic allocation', _gross_gwp_A * _oil_rev_frac, f'{_oil_rev_frac*100:.1f}% revenue fraction'),
    ('No credits (gross)', _gross_gwp_A, 'All impacts to oil; no credits'),
    ('Case B (EtOH 95% recovery, syst. exp.)', _R.get('GWP_kg_CaseB',0), 'Same as base but lower EtOH'),
]
pd.DataFrame(_alloc, columns=['Allocation method','GWP100 (kg CO₂-eq/kg oil)','Notes']).to_csv(
    os.path.join(OUT,'SI_Table_S8_LCA_allocation.csv'), index=False)
print("  ✓ SI Table S8 (LCA allocation sensitivity)")

# ── Table S9: EtOH recovery sensitivity ──────────────────────────────────────
_etoh_sens = []
for rec in [0.0, 0.50, 0.80, 0.90, 0.95, 0.99]:
    makeup = 180*_H*(1-rec)
    gwp_s  = makeup * 1.52
    gwp_tot= (_R.get('gwp_e',0)*1e3 + _R.get('gwp_t',0)*1e3 + gwp_s
              + _R['gwp_wu']*_R['oil_flow']*_H - abs(_R.get('gwp_ec',_R.get('gwp_cu',0)))*_R['oil_flow']*_H)
    gwp_kg = gwp_tot / (_R['oil_flow']*_H)
    _etoh_sens.append((f'{rec*100:.0f}%', makeup, f'{gwp_kg:.3f}',
                        'Base (current model)' if rec==0 else
                        ('Industrial target' if rec==0.95 else '')))
pd.DataFrame(_etoh_sens, columns=['EtOH recovery','EtOH charged (kg/yr)',
                                   'GWP100 (kg CO₂-eq/kg oil)','Note']).to_csv(
    os.path.join(OUT,'SI_Table_S9_EtOH_recovery_sensitivity.csv'), index=False)
print("  ✓ SI Table S9 (EtOH recovery sensitivity)")

# ── Table S10: Capacity scenarios ─────────────────────────────────────────────
import math
_cap_ref = 375.0  # 3000 t/yr base
_PCE_ref = _R['PCE']
_lang = 4.2
_sens10 = []
for _name, _cty in [('1,000 t/yr', 1000), ('3,000 t/yr (base)', 3000),
                     ('5,000 t/yr', 5000), ('10,000 t/yr (expansion)', 10000)]:
    _cph   = _cty * 1000 / 8000
    _scale = (_cph / _cap_ref)**0.6
    _pce   = _PCE_ref * _scale
    _cap   = _pce * _lang
    _fs_op = _cph * 8000 * 0.60     # feedstock OPEX
    _rev_e = _cph * (_R['oil_flow']/_cap_ref) * 8000 * 60  # ~oil revenue
    _note = ('Base case' if _cty==3000 else
             ('Requires strict governance' if _cty==10000 else ''))
    _sens10.append((_name, _cph, f'{_pce/1e6:.2f}', f'{_cap/1e6:.2f}',
                    f'{_fs_op/1e6:.2f}', f'{_rev_e/1e6:.1f}', _note))
pd.DataFrame(_sens10, columns=[
    'Scenario','Feed (kg/h)','PCE (M$)','CAPEX (M$)',
    'Feedstock OPEX (M$/yr)','Est. oil revenue (M$/yr)','Note']).to_csv(
    os.path.join(OUT,'SI_Table_S10_capacity_scenarios.csv'), index=False)
print("  ✓ SI Table S10 (capacity scenarios)")

# ── CHP energy balance table (Table 4.1 in manuscript) ───────────────────────
if 'chp_table' in _R:
    _chp = _R['chp_table']
    _chp_rows = [
        ('AD biogas methane',              f"{_chp['AD_CH4_kg_h']:.2f} kg/h", '50.05 MJ/kg',
         f"{_chp['AD_CH4_kg_h']*50050/3600:.1f} kW", 'CHP fuel'),
        ('Pyrolysis syngas methane',        f"{_chp['Pyro_CH4_kg_h']:.2f} kg/h", '50.05 MJ/kg',
         f"{_chp['Pyro_CH4_kg_h']*50050/3600:.1f} kW", 'CHP fuel'),
        ('Total fuel input',               f"{_chp['Total_CH4_kg_h']:.2f} kg/h", '50.05 MJ/kg',
         f"{_chp['Fuel_energy_kW']:.1f} kW", 'CHP'),
        ('Gross electricity output (η_e=35%)', '—', '—',
         f"{_chp['Gross_elec_kW']:.1f} kW", 'PRODUCT (before internal demand)'),
        ('Recoverable heat (η_h=45%)',         '—', '—',
         f"{_chp['Heat_recov_kW']:.1f} kW", 'Process heat (E-102, heating)'),
        ('Overall fuel utilisation',           '—', '—',
         f"{_chp['Fuel_util_pct']:.1f}%", 'η_total = η_e + η_h'),
    ]
    pd.DataFrame(_chp_rows, columns=['CHP item','Flow','LHV','Energy','Use/Destination']).to_csv(
        os.path.join(OUT,'SI_Table_CHP_energy_balance.csv'), index=False)
    print("  ✓ SI CHP energy balance table")

print("\n✓ All supplementary tables exported to:", OUT)
